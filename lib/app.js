...
app.get('/strains', async(req, res) => {
  try {
    ...
    from strains left join suppliers on strains.supplier_id=suppliers.id
    ORDER BY id
    `);
...
app.get('/strains/:id', async(req, res) => {
  try {
    ...
    on_sale,
    suppliers.supplier as supplier
    from coffees left join suppliers on coffees.supplier_id=suppliers.id 
    WHERE strains.id=$1
      `, [strainsID]);
...
app.put('/strains/:id', async(req, res) => {
  try {
    ...
    const data = await client.query(`
      ...
      RETURNING *
      `,
    [newNameID, newName, newImage, newDescription, newCategory, newPrice, newOnSale, newSupplierID, strainID]);
    res.json(data.rows[0]);
  } catch(e) {
